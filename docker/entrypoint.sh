...
sed -i '/$CONFIG = array (/ r /oidc.auth.php' /var/www/html/config/config.php
sed -i 's/${OIDC_CLIENT_ID}/'$OIDC_CLIENT_ID'/g' /var/www/html/config/config.php
sed -i 's/${OIDC_CLIENT_SECRET}/'$OIDC_CLIENT_SECRET'/g' /var/www/html/config/config.php
sed -i 's/${IAM_URL}/https:\/\/'$IAM_HOST'/g' /var/www/html/config/config.php

apt-get update
apt-get install gnupg wget -y
wget -q -O - "https://dist.eugridpma.info/distribution/igtf/current/GPG-KEY-EUGridPMA-RPM-3" | apt-key add -
echo "deb http://repository.egi.eu/sw/production/cas/1/current egi-igtf core" | tee -a /etc/apt/sources.list
apt-get update
apt-get install ca-policy-egi-core -y
...
tar -xf oidc_login.tar.gz -C /var/www/html/apps/
rm oidc_login.tar.gz
chown -R www-data:www-data /var/www/html/apps/oidc_login
chsh -s /bin/bash www-data
su - www-data -c 'php /var/www/html/occ app:enable oidc_login'
su - www-data -c 'export OC_PASS='$USER_PASS' && php /var/www/html/occ user:add --password-from-env --display-name="'$USER_DISPLAY_NAME'" --group="admin" '$USER_NAME
